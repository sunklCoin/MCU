...
struct pcb;
typedef void (_CODE_PTR_  PCB_FREE_FPTR)(struct pcb *);
...
extern uint32_t      ENET_initialize(uint32_t, _enet_address, uint32_t,    _enet_handle *);
extern uint32_t      ENET_initialize_ex(const ENET_PARAM_STRUCT *,  _enet_address,  _enet_handle *);
...
extern const char * ENET_strerror(uint32_t);
