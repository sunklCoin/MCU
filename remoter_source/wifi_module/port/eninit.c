   ...
   )
{
   ...
   //enet_ptr = _mqx_get_io_component_handle(IO_ENET_COMPONENT); //by colin
   ...
   //enet_ptr = _mem_alloc_system_zero(sizeof(ENET_CONTEXT_STRUCT));//by colin
   enet_ptr = qosal_malloc(sizeof(ENET_CONTEXT_STRUCT));
   if (NULL==enet_ptr)
      return ENETERR_ALLOC_CFG;
      
   //_mem_set_type((void *)enet_ptr, MEM_TYPE_IO_ENET_CONTEXT_STRUCT); //by colin
...
//by colin
...
#elif RTTHREAD_RTOS
   rt_sem_init(&enet_ptr->CONTEXT_LOCK,"contex_lock",1,RT_IPC_FLAG_FIFO);
#elif FREE_RTOS
   vSemaphoreCreateBinary(enet_ptr->CONTEXT_LOCK);
#endif   
   /* Initialize the MAC.*/
   result = (*param_ptr->ENET_IF->MAC_IF->INIT)(enet_ptr);
    ...
    if (ENET_OK == result)
    {
       ...
       // _mqx_link_io_component_handle(IO_ENET_COMPONENT,enet_ptr, (void **)&enet_ptr->NEXT);//by colin
		
		result = ENETERR_INITIALIZED_DEVICE;
        *handle = enet_ptr;
    } 
    else
    {
//by colin
        ...
        _mem_free(enet_ptr);
#elif RTTHREAD_RTOS
		rt_sem_delete(&enet_ptr->CONTEXT_LOCK);
#elif FREE_RTOS
		vSemaphoreDelete(enet_ptr->CONTEXT_LOCK);		
#endif   
		qosal_free(enet_ptr);
        *handle = NULL;
    }
